...
bool ImGui::CheckboxSlider(const char* label, bool* v)
{
    ImGuiWindow* window = GetCurrentWindow();
    if (window->SkipItems)
        return false;
    ...
    float hovered_id_timer = g.HoveredIdTimer;
    ...
    if (g.HoveredIdTimer == 0.f && g.HoveredId == id && !pressed)
        g.HoveredIdTimer = hovered_id_timer;

    float t = *v ? 1.0f : 0.0f;
    float t1 = hovered ? 1.0f : 0.0f;

    if (g.LastActiveId == id)
    {
        float anim = ImSaturate(g.LastActiveIdTimer / 0.05f);
        t = *v ? (anim) : (1.0f - anim);
    }
    ...
    const ImRect check_bb(pos, pos + ImVec2(height, height));
...
bool ImGui::Checkbox(const char* label, bool* v)
{
    ...
    window->DrawList->AddLine(ImVec2(pos.x, pos.y + 11), ImVec2(pos.x + 20, pos.y + 11), GetColorU32((held && hovered) ? ImGuiCol_FrameBgActive : hovered ? ImGuiCol_FrameBgHovered : ImGuiCol_FrameBg), 5);
...
bool ImGui::LineSliderScalar(const char* label, ImGuiDataType data_type, void* p_data, const void* p_min, const void* p_max, float scale, const char* format, ImGuiSliderFlags flags) {
    ImGuiWindow* window = GetCurrentWindow();
    if (window->SkipItems)
        return false;
    ...
    const float w = CalcItemWidth() * scale;
    ...
    const ImRect line_bb(pos.x, pos.y + height - 7, pos.x + w, pos.y + height - 5);
    ...
    const ImU32 frame_col = GetColorU32(g.ActiveId == id ? ImGuiCol_FrameBgActive : hovered ? ImGuiCol_FrameBgHovered : ImGuiCol_FrameBg);
    RenderNavHighlight(frame_bb, id);
    RenderFrame(line_bb.Min, line_bb.Max, frame_col, true, g.Style.FrameRounding);
    ...
    {
        window->DrawList->AddCircleFilled(ImVec2(grab_bb.Min.x + delta.x, grab_bb.Min.y + height - 8), (7 + t * 2), GetColorU32(ImGuiCol_FrameBgActive));
    }
    ...
    const ImRect val_bb(frame_bb.Min.x, frame_bb.Min.y - (CalcTextSize(value_buf, value_buf_end).y / 2) + 3, frame_bb.Max.x, frame_bb.Max.y);


    RenderTextClipped(val_bb.Min, val_bb.Max, value_buf, value_buf_end, NULL, ImVec2(0.f, 0.0f));
...
bool ImGui::SliderScalar(const char* label, ImGuiDataType data_type, void* p_data, const void* p_min, const void* p_max, float scale, const char* format, ImGuiSliderFlags flags)
{
    ImGuiWindow* window = GetCurrentWindow();
    if (window->SkipItems)
        return false;
...
bool ImGui::SliderFloat(const char* label, float* v, float v_min, float v_max, float scale, const char* format, ImGuiSliderFlags flags)
{
    return SliderScalar(label, ImGuiDataType_Float, v, &v_min, &v_max, scale, format, flags);
}

bool ImGui::LineSliderFloat(const char* label, float* v, float v_min, float v_max, float scale, const char* format, ImGuiSliderFlags flags)
{
    return LineSliderScalar(label, ImGuiDataType_Float, v, &v_min, &v_max, scale, format, flags);
}
...
bool ImGui::SliderInt(const char* label, int* v, int v_min, int v_max, float scale, const char* format, ImGuiSliderFlags flags)
{
    return SliderScalar(label, ImGuiDataType_S32, v, &v_min, &v_max, scale, format, flags);
}

bool ImGui::LineSliderInt(const char* label, int* v, int v_min, int v_max, float scale, const char* format, ImGuiSliderFlags flags)
{
    return LineSliderScalar(label, ImGuiDataType_S32, v, &v_min, &v_max, scale, format, flags);
}
